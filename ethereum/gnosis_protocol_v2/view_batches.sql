...
WITH batch_counts AS (
    SELECT s.evt_block_time,
           ...
           sum(case when selector != '\x2e1a7d4d'::bytea and selector != '\x095ea7b3'::bytea then 1 else 0 end) as dex_swaps,
           sum(case when selector = '\x2e1a7d4d'::bytea then 1 else 0 end)                               as unwraps,
           sum(case when selector = '\x095ea7b3'::bytea then 1 else 0 end)                               as token_approvals
     ...
     batch_values as (
         ...
         from gnosis_protocol_v2."view_trades"
                  JOIN prices.usd as p
                       ON p.contract_address = '\xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'::bytea
