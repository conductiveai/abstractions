...
WITH rows AS (
    ...
    FROM (
        ...
        SELECT
            ...
            trace_address,
            evt_index
        FROM clipper."view_trades" WHERE project='Clipper'
    ) dexs
...
VALUES ('*/10 * * * *', $$
    SELECT dex.insert_clipper(
        ...
        (SELECT max(number) FROM ethereum.blocks WHERE time < (SELECT max(block_time) - interval '1 days' FROM dex.trades WHERE project='Clipper')),
        (SELECT MAX(number) FROM ethereum.blocks where time < now() - interval '20 minutes'));
$$)
